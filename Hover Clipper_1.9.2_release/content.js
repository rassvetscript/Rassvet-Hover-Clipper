...
function createIndicator(link) {
  ...
  showCopiedUrlTooltip(text, markdown); 
...
function showCopiedUrlTooltip(url, isMarkdown = false) {
  if (activeCopiedUrlTooltip && activeCopiedUrlTooltip.parentElement) {
    ...
    setTimeout(() => {
      ...
      displayNewCopiedTooltip(url, isMarkdown); 
    }, 75);
  } else {
    displayNewCopiedTooltip(url, isMarkdown);
  }
...
function displayNewCopiedTooltip(url, isMarkdown = false) {
  ...
  tooltip.innerHTML = `
    <img src="${iconSrc}" alt="Copy" class="copy-icon">
    <span style="color:var(${colorVar});font-weight:bold;white-space: pre-wrap;">${url}</span>
  `;
  
  tooltip.classList.add('common-tooltip');
  tooltip.classList.add(isDark ? 'tooltip-dark' : 'tooltip-light');
